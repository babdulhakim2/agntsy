...
import { MOLTBOT_PORT, AGENTC_DAILY_MESSAGE_LIMIT } from './config';
import { createAccessMiddleware } from './auth';
import { ensureMoltbotGateway, findExistingMoltbotProcess, syncToR2 } from './gateway';
import { publicRoutes, api, agentcApi, adminUi, debug, cdp } from './routes';
import { resolveAgentCSession } from './agentc/middleware';
import { getUsageToday, incrementUsage, incrementConversationMessageCount } from './storage/d1';
import { appendMessage, type StoredMessage } from './storage/r2-user';
...
// Mount Agent C multi-user API (uses its own JWT session auth, not CF Access)
app.route('/api/v1', agentcApi);

// =============================================================================
// PROTECTED ROUTES: Cloudflare Access authentication required
// =============================================================================

// Middleware: Validate required environment variables (skip in dev mode, debug routes, and agentc routes)
app.use('*', async (c, next) => {
  const url = new URL(c.req.url);
  ...
  // Skip validation for Agent C routes (handled by agentcApi middleware)
...
// (Agent C /api/v1 routes are already mounted and use their own auth)
app.use('*', async (c, next) => {
  const url = new URL(c.req.url);

  // Skip CF Access for Agent C routes (they use session JWT auth)
  if (url.pathname.startsWith('/api/v1')) {
    return next();
  }
...
app.all('*', async (c) => {
  ...
  if (isWebSocketRequest) {
    ...
    // ── Agent C per-user WebSocket path ──────────────────────────────────
    if (sessionToken && c.env.AGENTC_JWT_SECRET) {
      console.log('[WS:agentc] Per-user WebSocket request');

      const agentcUser = await resolveAgentCSession(
        sessionToken,
        c.env.AGENTC_JWT_SECRET,
        c.env.AGENTC_DB
      );
      if (!agentcUser) {
      ...
      const usageToday = await getUsageToday(c.env.AGENTC_DB, agentcUser.id);
      if (usageToday >= AGENTC_DAILY_MESSAGE_LIMIT) {
      ...
      console.log('[WS:agentc] User:', agentcUser.email, 'Device:', agentcUser.deviceId, 'Conv:', conversationId);

      // Build a gateway-targeted WebSocket request (strip agentc params, add gateway token)
      const gwUrl = new URL(request.url);
      gwUrl.searchParams.delete('session');
      gwUrl.searchParams.delete('conversation');
      if (c.env.MOLTBOT_GATEWAY_TOKEN) {
        gwUrl.searchParams.set('token', c.env.MOLTBOT_GATEWAY_TOKEN);
      }
      const gwRequest = new Request(gwUrl.toString(), {
        headers: request.headers,
      });

      const containerResponse = await sandbox.wsConnect(gwRequest, MOLTBOT_PORT);
      const containerWs = containerResponse.webSocket;
      if (!containerWs) {
        console.error('[WS:agentc] No WebSocket from container');
      ...
      // Inject a connect frame to register this user as a device
      const connectFrame = JSON.stringify({
        type: 'req',
        id: `connect-${agentcUser.id}`,
        method: 'connect',
        params: {
          minProtocol: 1,
          maxProtocol: 1,
          client: {
            id: agentcUser.deviceId,
            displayName: agentcUser.displayName || agentcUser.email,
            platform: 'web',
          },
          role: 'operator',
          scopes: [],
      ...
      containerWs.send(connectFrame);
      console.log('[WS:agentc] Sent connect frame for device:', agentcUser.deviceId);

      // Helper to store a message in R2 (fire-and-forget)
      const storeMsg = (direction: 'in' | 'out', data: unknown) => {
        if (!conversationId) return;
        const msg: StoredMessage = { ts: new Date().toISOString(), direction, data };
        appendMessage(c.env.MOLTBOT_BUCKET, agentcUser.id, conversationId, msg).catch(err => {
          console.error('[WS:agentc] R2 write error:', err);
        });
      };

      // Client → Container relay with usage tracking
      serverWs.addEventListener('message', (event) => {
        if (containerWs.readyState === WebSocket.OPEN) {
          containerWs.send(event.data);
        }
        // Track outgoing messages for usage + storage
        if (typeof event.data === 'string') {
          try {
            const parsed = JSON.parse(event.data);
            storeMsg('out', parsed);
            // Count user-sent messages for rate limiting
            if (parsed.type === 'req' && parsed.method !== 'connect') {
              incrementUsage(c.env.AGENTC_DB, agentcUser.id).catch(() => {});
              if (conversationId) {
                incrementConversationMessageCount(c.env.AGENTC_DB, conversationId).catch(() => {});
              }
            }
      ...
      // Container → Client relay with error transform + storage
    ...
    // ── Existing admin WebSocket path (CF Access protected) ──────────────
